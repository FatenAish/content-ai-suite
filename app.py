...
# - Database-only answers (strict prompt)
# - Safe test mode (USE_DUMMY_LLM=1) — zero external calls
# - Clear Chat fixed via callback (no direct widget assignment)
# - Evidence preview, Rebuild Index, file download helper
# =========================================================
...
import difflib
import chardet  # noqa: F401 (kept for CSV enc autodetect tooling)
...
# ---- Safe test mode (no network) ----
class DummyLLM:
    def invoke(self, text):
        # Return the section after the marker to simulate a "model reply"
        marker = "ANSWER (STRICTLY FROM CONTEXT):"
        if isinstance(text, dict):
            # In case a dict reaches here (shouldn't with current chain)
            text = str(text)
        return text.split(marker, 1)[-1].strip() if marker in text else text

def get_local_llm():
    """
    If USE_DUMMY_LLM=1, returns DummyLLM (no external calls).
    Otherwise you can use ChatGroq (requires GROQ_API_KEY).
    All knowledge still comes ONLY from the provided context.
    """
    if os.getenv("USE_DUMMY_LLM", "0") == "1":
        return DummyLLM()
    # Real model (formatting only; knowledge is from our CONTEXT)
...
# ==============================================
def _reset_app():
    # Clear chat & widget state keys (no direct assignment)
    st.session_state.pop("rag_history", None)
    if "user_q" in st.session_state:
        del st.session_state["user_q"]
    # Nudge URL to guarantee a fresh render on some setups
    try:
        st.query_params["nonce"] = str(uuid4())
    except Exception:
        pass
    # Rerun (fallback for older Streamlit)
    try:
        st.rerun()
    except Exception:
...
# ==============================================
# USER QUESTION (bound to a key we can clear)
# ==============================================
...
with mid:
    ...
    with c2:
        st.button("🧹 Clear Chat", use_container_width=True, on_click=_reset_app)
...
""")
        ...
        # Evidence: show top chunks for transparency
        ...
        if hits:
            ...
            for i, (doc, score) in enumerate(hits, 1):
                ...
                st.markdown(
                    f"<div class='evidence'><b>{i}.</b> score={score:.3f}\n\n{snippet}…</div>",
                    unsafe_allow_html=True
                )
# =========================================================
# app.py — Dubizzle Group AI Content Lab (Internal RAG Only)
